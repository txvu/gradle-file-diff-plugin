...
class FileDiffTask extends DefaultTask {
    ...
    def diff() {
        ...
        if (file1.get().size() == file2.get().size()) {
            ...
            File largestFile = file1.get().size() > file2.get().size() ? file1.get().size() : file2.get().size()
            diffResult = "${largestFile.toString()} was the largest file at ${largestFile.get().size()} bytes."
        }
